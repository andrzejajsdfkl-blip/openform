...
import { Loader2, Sparkles, LayoutPanelTop, GripVertical, Trash2, Save, FileJson, Settings2, PlusCircle, Check, Pencil } from 'lucide-react';
...
function TableConfigDialog({ table, onUpdateTable, onRemoveTable }: {
  ...
  const handleAddColumn = (type: 'text' | 'date' | 'int' | 'decimal' | 'bool') => {
    ...
    if (nextKey) {
      ...
      // Optionally, show a toast or alert that all columns of this type are used.
    }
  };
...
export default function NewType() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!documentType || !over || active.id === over.id) return;
  ...
  const handleSuggest = async () => {
    ...
    try {
      ...
      const fields: FieldConfig[] = suggestion.fields.map((f, i) => ({
        ...f,
        type: f.type as any,
        enabled: true,
        order: i + 1,
  ...
  const handleUpdateTable = (updatedTable: TableConfig) => {
    if (!documentType || !documentType.tables) return;
