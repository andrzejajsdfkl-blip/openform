...
// ─────────────────────────────────────────────
function CompactDocumentRow({ doc }: { doc: import('@/lib/types').DocumentInstance }) {
  const firstField = doc.fields.filter((f) => f.enabled).sort((a, b) => a.order - b.order)[0];
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 bg-card/40 hover:bg-card/70 border border-border/40 hover:border-primary/30 rounded-2xl transition-all group">
      ...
      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button variant="ghost" size="icon" className="h-7 w-7 rounded-xl" onClick={downloadJson}>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ background: 'linear-gradient(105deg, transparent 0%, rgba(255,255,255,0) 40%, rgba(255,255,255,0.06) 50%, transparent 60%)' }}
          ...
          <div className={viewMode === 'compact' ? 'space-y-1.5' : 'space-y-4'}>
            ...
            <AnimatePresence mode="popLayout">
              {filteredDocs.length > 0 ? (
                ...
                documentTypes.length > 0 ? (
                  // Scenario A: blueprints exist, vault empty
                  <motion.div
                    key="onboarding-a"
                  ...
                  >
                    <div className="text-center py-16 bg-card/20 backdrop-blur-xl border border-border/30 rounded-[3rem] p-8 shadow-2xl relative overflow-hidden">
                      ...
                      <p className="text-muted-foreground text-lg max-w-lg mx-auto mb-10">
                        You have defined {documentTypes.length} blueprint
                        {documentTypes.length > 1 ? 's' : ''}. Let&apos;s bring them to life by
